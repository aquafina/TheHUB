package ict.nishat.net.Servlet;

import ict.nishat.net.attendanceeventlogger.Job;
...
public class JobServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            JobDetail jd = new JobDetail(JOB_NAME, GROUP_NAME, Job.class);
            CronTrigger cronTrigger =
                new CronTrigger(TRIGGER_NAME, GROUP_NAME);
    ...
    public void doGet(HttpServletRequest request,
        ...
        if ("single".equals(action)) {
            if (sched != null) {
                try {
                    Trigger trigger =
                        new SimpleTrigger("SOASingleTrigger", GROUP_NAME,
                                          new Date());
            ...
            if (sched != null) {
                try {
                    JobDetail jd =
                        new JobDetail(JOB_NAME, GROUP_NAME, Job.class);
                    CronTrigger cronTrigger =
                        new CronTrigger(TRIGGER_NAME, GROUP_NAME);
                    // Get the cron Expression as an Init parameter
                    ...
                    cronTrigger.setCronExpression(cronExpr);
                    System.out.println(this.getClass().getName() +
                                       " Scheduling Job " + JOB_NAME);
                    sched.scheduleJob(jd, cronTrigger);
                    System.out.println(this.getClass().getName() + " Job " +
                                       JOB_NAME + " scheduled.");
                } catch (Exception e) {
                    System.out.println(this.getClass().getName() +
                                       e.getLocalizedMessage());
        ...
        try {
            ...
            for (i = 0; i < jobGroups.length; i++) {
                ...
                for (j = 0; j < jobsInGroup.length; j++) {
                    ...
                    jobTriggers =
                            sched.getTriggersOfJob(jobsInGroup[j], jobGroups[i]);
